...
const savedReviews = JSON.parse(localStorage.getItem("reviews")) || [
  {
    ...
    review:
      "Ой я в полном восторге. Мне казалось что я никогда не разберусь с этими артиклями. Теперь я шарю!))))",
    rating: 5,
...
const getReview = (review) => {
  ...
  const relatedDate = formatDistance(new Date(created), new Date(), {
    locale: ru,
    addSuffix: true,
    includeSeconds: true,
  });
...
const generateHTML = () => {
  const reviewsHTML = savedReviews
    .map((review) => {
